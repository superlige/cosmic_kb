...
>
> **国内无 VPN、访问 pypi.org 慢或超时**：在命令末尾加国内镜像即可，例如
> `pip install -e ".[parse,mcp]" -i https://pypi.tuna.tsinghua.edu.cn/simple`（清华，
> 或阿里云 `https://mirrors.aliyun.com/pypi/simple/` 等任一）。想省掉每次 `-i`，可先
> `pip config set global.index-url https://pypi.tuna.tsinghua.edu.cn/simple` 把镜像设成默认源。
