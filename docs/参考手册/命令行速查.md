...
| `cosmic_kb callers "Class.method"` | Java 方法反查：全部静态调用点（含 `Class::method`）+ 符号覆盖率；简单类名重名会反问 |
...
`coverage`/`scan-compare`/`dynwrites`/`source` 只在 CLI 提供；MCP 精简暴露给 agent 5 个主
路径工具（`trace`/`bill`/`resolve_fields`/`callers`/`cosmic_semantics`）。
