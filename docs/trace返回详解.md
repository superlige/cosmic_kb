...
groups[] / groups_total / groups_capped
                            按坐标(单据·层级·分录)分组；裸字段命中多单据时 groups 被裁，
                            真实组数 = groups_total，被截 = groups_capped
...
4. **dynamic_writers / unlocated 是工作单，不是结论。** 它们按方法去重列出"该读方法"，要把"谁改了 X /
   来自哪张单据"答全，对清单逐个 `method_calls` / 读本机源码核实，别把 `plugin_form_label` 当确诊。
...
- **无界段也逐档收紧**：坐标组 `groups` / `occurrences` / `dynamic_writers` / `coarse` 本身无界
  （裸字段可命中十几张单据），是"全量调"撑爆的主因——governor 的 ladder 对它们也逐级裁剪，否则
  per-class cap 再小也压不下整组体积（这是旧版无法收敛、仍被 ③ 切的洞）。
