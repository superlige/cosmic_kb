...
def resolve_fields(conn, keys: list[str], *, kind: str | None = None) -> dict[str, Any]:
    ...
    `kind`：`field`/`entity`/`form` 之一，给定时只返回对应种类的候选（issue 4）——模型自己读到
    `.loadSingle(id, key)` 这类单据字面量时传 `kind="form"`，从根上避免混入同 key 的字段候选噪声。
    """
    ...
    for key in keys:
        ...
        if qualified is None:
            resolved[key] = _items_for(key, lex, kind=kind) or None
            continue
