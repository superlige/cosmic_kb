...
| 谁调用了某 Java 方法、方法引用位置、死代码验证（含入口可达性：自动回溯插件事件入口 + 反查元数据注册/启用状态） | `callers`；简单类名歧义时按 candidates 改传完整 FQN；死代码判定看 `entry_analysis.verdict`，不再只看调用点数为 0 |
...
1. 用户问“谁调用了这个方法”“这个方法/类是不是死代码”时，先用 `callers`；翻看每条 resolution 与
   顶层 `resolution_coverage`，符号层不可用/覆盖不足时不得把 0 结果断言成死代码。死代码/可达性
   结论以 `entry_analysis.verdict` 为准（它已自动回溯到插件事件入口并反查元数据注册/启用状态，
   不必再手动拼调用链）：`entry_reachable` 才是"确认可达"，`entries_inactive` 只是"疑似不可达"
   （likely，不排除反射/动态注册），`entry_unverifiable`/`no_entry_found` 是弱证据，都不得写成
   "已确认死代码"；问题聚焦可达性/死代码判定时按“调用链反查/死代码判定结论”模板输出，不套用
   下面第 7 步的业务作用解释模板。
...
7. 问题聚焦这个方法/插件**做什么业务**（而非是否可达）时，对不确定的插件事件、SDK 或保存语义
   调用 `cosmic_semantics`，再按“插件/方法作用解释”模板输出。
...
| `entry_analysis.verdict: entry_reachable` | 已找到已注册且启用的插件入口，静态可达 |
| `entry_analysis.verdict: entries_inactive` | 找到的入口均已禁用或未注册，疑似不可达（不排除反射/动态注册） |
| `entry_analysis.verdict: entry_unverifiable` | 注册/启用状态无法确认，或负向结论因截断/符号层弱化被降级，不足以断言 |
| `entry_analysis.verdict: no_entry_found` | 静态追不到任何插件事件入口 |
| `entry_analysis.verdict: not_analyzed` | 目标是平台/JDK 类，未做入口可达性分析 |
| `registration.status: registered_enabled` | 元数据已注册且至少一处有效绑定已启用 |
| `registration.status: registered_disabled` | 元数据已注册但全部有效绑定已禁用 |
| `registration.status: registered_enabled_unknown` | 已注册，但启用状态未知（NULL 或规则悬空） |
| `registration.status: orphan_unregistered` | KB 有该类插件的注册表，但查无绑定（未注册） |
| `registration.status: orphan_unverifiable` | KB 未接入该类插件的注册配置表（如调度计划/开放平台/工作流），无法确定是否平台注册 |
...
用户问“谁调用了这个方法”“这个方法/类还有没有用”“这段代码敢不敢删”时用本模板，而不是“插件/方法作用
解释”模板——这里的结论重点是**可达性**，不是业务语义。`entry_analysis` 已经把静态调用链自动回溯到
插件事件入口、并反查了元数据注册与启用状态，不要只看 `callers` 的调用点数量下结论。

````markdown
方法：<Class.method>
调用点：<call_sites 数量>（含方法引用 <method_references> 个）｜符号覆盖：<resolution_coverage 的
coverage 百分比 + status；是否达到强证据线>｜分页：<pagination.complete 已完成/未完成>

可达性结论：<verdict 中文表达，见术语对照表>（<confidence 中文：已确认/可能/未知>）

### 入口清单（按类去重）
| 入口/边界类 | 到达方式 | 事件（若有） | 注册状态 | 绑定单据/操作 |
|---|---|---|---|---|
| <类全限定名> | <本方法即入口/已回溯到事件入口/仅到插件类边界，事件方法未确认> | <event/phase，没有写“无”> | <已注册且启用/已注册但已禁用/已注册·启用状态未知/该类插件 KB 有注册表但查无绑定(未注册)/该类插件 KB 未接入注册表(无法确定)> | <单据（中文名）+ 操作 key；没有写“无”> |

代表性入口链（最多列 3 条，来自 `entry_analysis.chains`）：
<Class#method@file:line → Class#method@file:line → …；heuristic 边命中的链已在可达性结论里降级为“可能”>

调用点明细（按调用方法去重，最多列 3 个代表锚点）：
- <caller 类#方法>  <file:line>  [resolution/confidence]  <另有 N 处>

能力边界：<本次真实出现的截断/符号层弱化/未覆盖反射与动态派发等边界；“可能不可达”不等于“确认死代码”，
没有真实边界不要照抄反射/动态注册等示例词>
下一步建议：<verdict=entry_unverifiable/no_entry_found 时给出需要人工读源码核实反射/动态注册的具体
类或调用点；verdict=entries_inactive 时提醒删除前仍需人工确认没有反射/动态触发；verdict=entry_reachable
时可直接给出结论>
````
...
- “能力边界”只陈述静态程序化证据属性和本次真实出现的中断/截断，不照抄示例类名或泛列未出现的
  反射、任务、工作流等类别。
- `callers` 的 0 结果已结合“解析覆盖率”分强/弱证据，没有在符号层降级时断言死代码；死代码/
  可达性结论已看 `entry_analysis.verdict`（五值），`entries_inactive`/`entry_unverifiable`/
  `no_entry_found` 都不是“已确认死代码”，`task`/`webapi`/`workflow` 等 KB 未接入注册表的
  插件种类归 `entry_unverifiable`，没有被当成已确认可达或已确认停用。
- 每条关键结论都有工具结果或 `文件:行号`。
...
- **术语检查**：输出文字和 mermaid 图里**<font color=red>没有</font>**原样出现 `self_entry`/`terminal`/`no_static_caller`/
  `plugin_boundary`/`confirmed`/`likely`/`unknown`/`coarse_only`/`entry_reachable`/
  `entries_inactive`/`entry_unverifiable`/`no_entry_found`/`not_analyzed`/
  `registered_enabled`/`registered_disabled`/`orphan_unregistered`/`orphan_unverifiable` 等
  工具内部标识——都已按“术语对照”表换成中文表达。
