...
.PARAMETER RefreshOnly
  只按当前版本刷新 README / 安装说明.md 的安装口令，不生成离线包。
  供 scripts\release.ps1 在提交前调用。
...
2) 装固定版本 cosmic-kb[complete]（含 parse/encoding/mcp/postgres；底层库若是 Oracle 改装 cosmic-kb[complete,oracle]）。
...
5) 若直连底层库取元数据，加 --db-config 与 --prompt-db-password：数据库口令只能在终端隐藏输入，绝不要我贴进对话，也不写进任何命令/配置/日志。底层库类型在配置文件 driver 字段选：postgresql（默认，端口 5432）或 oracle（端口 1521，database 填 service 名、schema 填对象 owner）。
...
function Update-InstallToken([string]$Path, [string]$Block) {
    ...
    # Windows PowerShell 5.1 会把无 BOM 的 UTF-8 当成本地 ANSI 读取，随后写回就会整篇乱码。
    # 仓库 Markdown 统一是 UTF-8 无 BOM，因此这里必须显式按 UTF-8 读取。
...
if ($RefreshOnly) {
    Write-Host "✓ 安装口令已同步到版本 $Version" -ForegroundColor Green
    return
}
...
$required = @(
    ...
    "cosmic_kb/java/vendor/symsolver.jar"
)
...
try {
    ...
    Write-Host "  资产：  两份 Agent Skill、semantics 已含"
