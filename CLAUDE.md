...
- ✅ 阶段 9–10：`ask`（NL→意图→确定性证据包）+ MCP 封装，工具面 `ask`/`trace`/`bill`/`resolve_fields`/`cosmic_semantics`（5 个）。
...
- ⬜ 阶段 8（业务流）拍板搁置，折入阶段 9 按需；阶段 11（增量重扫+GitNexus）待开发。

当前 schema **v14**；`pytest -q` 结果以 codex 最近一次全量执行为准。详细清单/每条验收结论见
`docs/核心/阶段验收.md`。
