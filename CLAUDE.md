...
- ✅ 阶段 12（编译期符号解析，3 子阶段全部完成）：12.1 ✅ 类路径发现（IDEA/.iml + Gradle 模板 + 显式兜底，
  ...
  后续加固 callers 入口可达性/死代码判定（entry_analysis：verdict 五值 + 元数据注册反查）。
- ⬜ 阶段 8（业务流）拍板搁置；阶段 11（增量重扫+GitNexus）待开发。

当前 schema **v20**（v19 新增全量 `call_edge` 与正反索引供 `callers` 反查；
v20 新增 `field_access.access_method`）；
...
cosmic_kb callers "ContractService.updateRlateAssets"  # 反查调用点 + 回溯插件事件入口并反查注册/启用状态（苍穹入口可达性/死代码判定）；0 结果必带符号覆盖率，降级口径不足以断言死代码；entry_analysis.verdict 五值见阶段验收表
