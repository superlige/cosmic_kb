...
> **底层库直连支持 PostgreSQL 与 Oracle**：`complete` 自带 PostgreSQL 驱动（psycopg2）；底层库是 **Oracle** 时把安装包名换成 `cosmic-kb[complete,oracle]`（走 python-oracledb thin，无需 Instant Client），并在 `cosmic_db.json` 里把 `driver` 设为 `oracle`（默认 `postgresql`；Oracle 端口 1521、`database` 填 service 名、`schema` 填对象 owner 通常大写用户名）。详见 [`建库与更新详解`](docs/参考手册/建库与更新详解.md)。

## 升级到新版本

**对话式升级，和装一样简单**：安装口令是版本锁定的，每次发版口令里的版本号会自动刷新——升级就是**把上面这段最新的安装口令整段再发给你的 agent**。它会把新版本装进同一个用户级隔离运行时 `%USERPROFILE%\.cosmic_kb\runtime`（pip 装固定版本＝原地升级包，不新建环境），再重跑 `bootstrap apply`（幂等、断点续跑）：刷新版本清单 → 复装 Skill → doctor → 注册/校验 MCP。

升级后有两件 `apply` **不会自动替你做**、需要留意的事：

- **发版说明提示要重建 KB 时**（新版本升了 KB schema），`apply` 默认「KB 已存在就跳过建库」不会重建；这时补一句「新版本了，帮我重建 KB」，或让 agent 跑 `cosmic_kb bootstrap apply --rebuild`。
- **重连 / 重启 agent**：MCP 是常驻进程，升级后必须重连才加载新代码（`apply` 收尾也会提醒）。

> 一句话：**发新版口令 → 需要时补「帮我重建 KB」→ 重连 agent**。国内慢就照前面说的把源换清华镜像。
...
- [`V0.2.2 发版说明`](docs/核心/V0.2.2发版说明.md) —— 新增 Oracle 只读元数据源与一键发版流程
