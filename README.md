...
纯本地运行、不外传源码；专治「野生」苍穹项目：多模块、可能不可编译、缺依赖、多 ISV 前缀。
...
**①** 把元数据和 Java 源码**一起解析进同一份 KB**，查字段/单据靠查库，不用
再肉眼来回翻文件、自己拼证据；扩展单据的原厂继承字段也能自动补齐，不会结构性半盲（见「第一
步」）。

**②** 把苍穹这套私有二开框架的**领域知识**（插件类型/事件时机/原厂 SDK 用法）也备进
KB——最大作用是**防止大模型分析源码时乱猜**：通用 agent 见到苍穹代码只会套一套普通
Java/Spring 经验硬编业务逻辑，猜错了还不自知；接上本工具的 MCP 后，agent 会先查权威语义文档
再下结论。

工具本身**不调用大模型、不下结论**——只产出确定性证据（含苍穹领域语义文档原文），讲成人话
靠你接的 agent（见下）。接手一个陌生苍穹老项目，没有本工具时大概率是这样排障的：

**字段级排障** `trace "单据.字段"` —— 一次列出所有读写它的插件/方法/事件/是否落库/源码行号。
...
**单据钻取** `bill "单据标识"` —— 一次列出表单插件/操作/每个操作绑定的插件，三类插件全在
一份结果里。

> 没有时：苍穹设计器里这三处分散在不同页面（插件绑定页/具体操作/列表设计视图），来回切
> 好几个视图才能拼起来。

**标识核对** `resolve_fields` —— agent 读到字段/分录/单据这类英文标识时，一次核对出真实
中文名 + 元数据定义（所属分录层级、字段类型、是否基础资料引用等），查不到/有歧义如实说，
绝不瞎编。

> 没有时：大模型读到 `cqkd_zkd`、`srctransid` 这类英文标识，只能按命名习惯或拼音瞎猜中文
> 含义和类型（真实翻车：「转款单」被猜成"转账单"），容易凭表面写法编一套业务含义。

**苍穹领域语义** `cosmic_semantics` —— 查权威文档：插件类型、事件触发时机、原厂 SDK 用法、
入库判断规则、反模式黑名单。
...
**信任优先** `coverage` / `scan-compare` —— 扫描覆盖率和精度摆出来，可信度是一等公民而非
事后补。
> 没有时：分析结论没法验证是不是漏扫了，漏掉的地方容易被悄悄当成"没有这个逻辑"。
...
| 类别 | 拉取范围 | 更新逻辑 |
|---|---|---|
| **你自己的二开**（form/entity/转换规则，`fisv` = 你项目自己的实施方标识） | 首次 `build`：**全量**拉该 ISV 下所有记录；之后再跑 `build`：只拉**变更过的** | 按 `fmodifydate` 判定："上次同步时间"存在 KB 里，之后每次只拉 `fmodifydate > 上次时间` 的记录，**同 key 整条替换**（不是字段级合并，删掉/改名的字段不会残留） |
| **原厂标准单据**（继承母体，如 `bd_customer`） | 每次 `build` 都按三类确定性信号（扩展母体 / ORM 查询 / 操作执行）**自动发现**代码里实际引用到的 key，现取这些 key 的最新值 | 不做时间戳增量跟踪，因为每次都是精确按需重新现取这几个 key，天然就是最新的；不会不加区分地拉库里所有原厂表单（原厂页面太多，会引入大量噪声） |

也就是说：
- **日常重扫**（源码或元数据有更新后），原地重跑同一条命令即可，自己的二开部分会自动只拉
  变更、原厂部分照旧精确现取：
  ```powershell
  cosmic_kb build "D:\项目源码根" --db-config cosmic_db.json
  ```
- **想无视上次同步时间、强制重新全量拉一遍自己的二开元数据**，加 `--full-refresh`：
  ```powershell
  cosmic_kb build "D:\项目源码根" --db-config cosmic_db.json --full-refresh
  ```
...
[`docs/返回值字段词典.md`](docs/返回值字段词典.md)）；`cosmic_kb --help` / `cosmic_kb <子命令> --help`
...
- [`docs/返回值字段词典.md`](docs/返回值字段词典.md) —— 每个工具返回字段的详细含义
- [`docs/trace返回详解.md`](docs/trace返回详解.md) —— `trace` "完整 vs 截断" 的完整推演
- [`docs/分发与多agent接入方案.md`](docs/分发与多agent接入方案.md) —— 分发/跨 agent 接入的设计决策
- [`docs/开发计划.md`](docs/开发计划.md) / [`docs/阶段验收.md`](docs/阶段验收.md) —— 分阶段交付蓝图与验收记录
